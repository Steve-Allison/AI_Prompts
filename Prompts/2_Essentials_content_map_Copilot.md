...
## (version 3.1 – adds Supported‑File rules, Learning‑Goal/Objective alignment, Course‑Structure authority, and explicit folder‑scan directive)_
...
| **Delivery Mode → `mode`**             | First heading/table cell matching `/Modality   |
| **Assessment Plan → `assessment`**     | First heading/cell matching `/Assessment       |
| **Target Persona → `learner_profile`** | Extract from `\*\_\_Learner_Personas.md`       |

When extracting **Duration**, match any field labelled _Duration_ or _Suggested Timings_ (case‑insensitive).  
...
Learner*Outcomes_Module · Learning_Activity_Generator_Module · Learning_Asset_Module · Pedagogy_Module · Differentiation_Scaffolding_Module · Misconception_Check_Module · Accessibility_Module · Tone_of_Voice_Module · Visual_Style_Module  
\_Optional:* Learning_Theories_Checklist_Module · Cognitive_Keyword_Watchlist_Module
...
4. **Map** validated content to the eight modules **and, for each module, immediately**:
   • call `Learner_Outcomes_Module` to generate the Learning Outcomes;
   • call `Learning_Activity_Generator_Module` to generate Activities/Assessments.
   The results are written straight into the Layer 1 table.

   **4 a. Learning‑Goal & Objective Alignment Rule** – Every Key Topic/Sub‑topic must align explicitly with the locked Goal and Objectives (verbatim). If no alignment, omit or error‑list. Add Objective ID in brackets after each Learning Objectives.
...
5. **Align** items to Bloom, Gagné, Kirkpatrick, ARCS tags (details recorded in Appendix).
6. **Accessibility pre‑check** (Accessibility_Module) – halt if WCAG fails.
7. **(Step merged into 4)** Activities & assessments are already generated per module; now run Differentiation_Scaffolding_Module → Misconception_Check_Module to enrich those entries.
...
11. Do not invoke any export or formatting routines (e.g. DOCX generation) until Step 9 is complete and all content is final.
...
- **Learning Goal:** [populated from learning-design input]
- **High-level Objectives:** [populated from learning-design input]

### 6.2 Layer 1 – Content‑Map Table

**Populate `Activities/Assessments` and `Learning Outcomes` in real time for each module row by invoking `Learning_Activity_Generator_Module` and `Learner_Outcomes_Module` immediately after mapping topics and sub‑topics. No post‑processing step is needed; the table must be complete on first output.**

Module ID · Module Title · Duration · Key Topics · Sub‑topics · Learning Objectivess (with ID) · Activities/Assessments · Learning Outcomes

### 6.3 Layer 2 – Appendix

Present the Appendix **as a single Markdown table** with the columns listed below.  
**Validation rule:** Appendix _Key Topic_ and _Sub‑topic_ labels must replicate the Layer 1 Content‑Map table **verbatim and in identical order**. If any mismatch is detected, **halt** and list the discrepancies.
...
_For every row, populate all applicable columns (include the numeric Module ID as the first column). Do **not** merge items across modules or topics. Use the exact module titles from `Essentials_course_structure` as the Module values._
